...
function Connector({ position, children, vec = new THREE.Vector3(), scale, r = THREE.MathUtils.randFloatSpread, accent, ...props }) {
  ...
  return (
    <RigidBody linearDamping={2} angularDamping={0.7} friction={0.2} position={pos} ref={api} colliders={false}>
...
function Model({ children, color = 'white', roughness = 0, ...props }) {
  ...
  useFrame((state, delta) => {
    ...
    ref.current.rotation.x = ref.current.rotation.y += delta * 0.2
  })
  return (
    <mesh ref={ref} castShadow receiveShadow scale={2} geometry={nodes.star.geometry || Object.values(nodes)[0].geometry}>
      <meshStandardMaterial metalness={0.4} roughness={roughness} {...props} />
      {children}
    </mesh>
  )
}
